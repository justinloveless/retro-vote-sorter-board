...
public class SupabaseGateway : ISupabaseGateway
{
    ...
    public async Task<AvatarUploadResponse> UploadAvatarAsync(string bearerToken, string userId, byte[] bytes, string contentType, string? correlationId = null, CancellationToken cancellationToken = default)
    {
        try
        {
            var fileName = $"{userId}.png"; // keep consistent naming

            var request = new HttpRequestMessage(HttpMethod.Post, $"/storage/v1/object/avatars/{fileName}");
            // Use multipart/form-data to match supabase-js behavior and enable upserts
            ...
            // Build absolute Storage API URL from PostgREST base
            ...
            if (!resp.IsSuccessStatusCode)
            {
                throw new HttpException(resp.StatusCode, $"Supabase storage upload failed with status {resp.StatusCode}");
            }
