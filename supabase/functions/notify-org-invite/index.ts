...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: invitation, error: invError } = await supabase
      ...
      .select('id, organization_id, email, invited_by, token, status, is_active')
      .eq('id', invitationId)
      .single();
    ...
    const url = `/org-invite/${invitation.token}`;
    const { error: insertError } = await supabase
      ...
      .insert({
        ...
        message: `You have been invited to join the organization "${org?.name || 'an organization'}".`,
        url
      });
