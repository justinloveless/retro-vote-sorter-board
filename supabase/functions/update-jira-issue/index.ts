...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!hasDescription && !hasAssignee && !hasSummary && !hasPriority && !hasIssueType && !hasTransition) {
      throw new Error(
        'Provide at least one of: description, assigneeAccountId, summary, priorityId, issueTypeId, transitionId',
      );
    }
    ...
    // Use REST v2 for issue updates so `description` is wiki/HTML string (same as get-jira-issue GET).
    // v3 ADF + one-paragraph-per-line breaks wiki macros like {panel} and can produce \{ escapes in Jira.
    const jiraBaseUrl = `${jira_domain}/rest/api/2/issue/${encodeURIComponent(resolvedKey)}`;
