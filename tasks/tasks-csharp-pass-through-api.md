...
| 21 | Progressive coverage tracker setup | Create and maintain `documentation/plans/csharp-api-coverage-tracker.md`. Add rows for each Supabase usage. | Done | - | ✅ Tracker created with comprehensive coverage map. Updated initial 3 endpoints to "Covered" status. |
| 22 | Phase 2: Notifications parity | Implement mark-as-read and mark-all-read endpoints with tests and FE migration behind flag. | Done | 6,7,10 | ✅ Complete TDD implementation: DTOs → unit tests → controllers → gateway → integration tests → FE migration. All 25 integration tests passing. Frontend hooks migrated behind feature flag with backward compatibility. |
| 23 | Phase 3: Teams & Members | Add teams CRUD and member add/remove/role endpoints, tests, and FE migration. | Done | 22 | Implemented controllers, gateway methods, unit + integration tests; FE hooks behind flag. |
| 24 | Phase 4: Retro boards | Add retro endpoints (CRUD, columns, items, comments, votes, grouping, exports), tests, and FE migration. | In progress | 23 | See plan Section C Phase 4. Start with DTOs and controller tests. |
