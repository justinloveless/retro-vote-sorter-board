import { isJiraBrowseBacklogSprint } from '@/lib/jiraBrowseSprintBuckets';
...
/** Display label + id for the sprint bucket (matches Browse Jira grouping). */
export function getSprintBucketFromIssueFields(fields: Record<string, unknown> | null | undefined): {
  displayName: string;
  sprintId: number | null;
} {
  const emptyName = new Map<number | string, string>();
  const emptyStart = new Map<number | string, string>();
  const meta = createGetSprintMeta(emptyName, emptyStart)(fields);
  const name = meta.name;
  const displayName =
    name == null || isJiraBrowseBacklogSprint(name) ? 'Backlog' : name;
  return { displayName, sprintId: meta.sprintId };
}
