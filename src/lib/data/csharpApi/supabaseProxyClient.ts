...
import { getApiBaseUrl, getCSharpApiHeaders } from '../../../config/environment';
  ...
  private async execute(): Promise<SupabaseResponse<T>> {
    try {
      ...
      const requestHeaders: Record<string, string> = {
        ...this.headers,
        ...getCSharpApiHeaders(), // Include debug headers
      };
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return {
          ...
          error: {
            message:
              errorData.message || `Function invocation failed with status ${response.status}`,
            details: errorData.details,
            hint: errorData.hint,
            code: errorData.code || String(response.status),
          },
...
export class SupabaseProxyClient {
  ...
  constructor(accessToken?: string | null) {
    ...
    this.auth = new SupabaseProxyAuthClient(this.accessToken, token => this.setAccessToken(token));
  }
